...
export const packageUpload = (req: Request, res: Response, next: NextFunction): void => {
    const bb = busboy({ headers: req.headers });
    const resource: Resource = { id: '', type: '', path: '', jsons: [], runners: [] };

    let fileResolve: (value?: unknown | PromiseLike<unknown>) => void;
    const filePromise = new Promise<unknown>(resolve => { fileResolve = resolve; });
    ...
    eventHandler('file', (name: string, file: fs.ReadStream, info: { encoding: string; filename: string; mimeType: string }) => {
        const { mimeType, filename } = info;

        if (mimeType !== 'application/x-zip-compressed' && mimeType !== 'application/zip') {
            return errorHandler(new AppError('Please upload a zip file', 404));
        }

        const appLocation = path.join(appsDirectory, resource.id);
        resource.path = appLocation;

        fs.mkdtemp(path.join(os.tmpdir(), `metacall-faas-${resource.id}-`), (err, folder) => {
            if (err) return errorHandler(new AppError('Failed to create temporary directory for the blob', 500));

            resource.blob = path.join(folder, filename);

            ensureFolderExists(appLocation)
                .then(() => {
                    file.pipe(fs.createWriteStream(resource.blob as string)).on('finish', () => {
                        fileResolve();
                    });
                })
                .catch((error: Error) => {
                    errorHandler(new AppError(`Failed to create folder for the resource at: ${appLocation} - ${error.toString()}`, 404));
                });
        });
    });

    eventHandler('field', (name: keyof Resource, val: string) => {
        if (name === 'runners') resource.runners = JSON.parse(val) as string[];
        else if (name === 'jsons') resource.jsons = JSON.parse(val) as MetaCallJSON[];
        else resource[name] = val;
    });

    const deleteBlob = () => {
        if (resource.blob !== undefined) {
            fs.unlink(resource.blob, error => {
                if (error) errorHandler(new AppError(`Failed to delete the blob at: ${error.toString()}`, 500));
            });
        }
    };
    ...
    const deleteFolder = () => {
        if (resource.path !== undefined) {
            fs.rm(resource.path, { recursive: true, force: true }, error => {
                if (error) errorHandler(new AppError(`Failed to delete the path at: ${error.toString()}`, 500));
            });
        }
    };

    eventHandler('finish', () => {
        if (!resource.blob) throw new Error('Invalid file upload, blob path is not defined');

        if (Applications[resource.id]) {
            deleteBlob();
            return errorHandler(new AppError(`There is an application with name '${resource.id}' already deployed, delete it first.`, 400));
        }
        ...
        Applications[resource.id].resource = new Promise<Resource>((resolve, reject) => {
            ...
            return new Promise<void>((resolve, reject) => {
                fs.createReadStream(resource.blob as string)
                    ...
                    .on('error', error => {
                        deleteBlob();
                        deleteFolder();
                        reject(new AppError(`Failed to unzip the resource at: ${error.toString()}`, 500));
                    });
            });
        };
