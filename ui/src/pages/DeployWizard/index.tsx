...
export default function DeployWizardPage() {
  ...
  if (!file) {
    return (
      <div className="flex-grow flex flex-col items-center justify-center p-4 min-h-[calc(100vh-80px)] bg-slate-50/50">
        <div className="flex flex-col items-center gap-2 text-center max-w-sm bg-white p-8 border border-gray-200 shadow-sm rounded-md">
          ...
          <button
            ...
            className="w-full px-6 py-2.5 bg-slate-900 text-white text-sm font-medium rounded hover:bg-slate-800 transition-colors shadow-sm"
          >
            Go to Deploy Hub
          </button>
        </div>
              ...
              {loadingZip ? (
                ...
                <div className="flex flex-col gap-1">
                  ...
                  >
                    ...
                    <TreeCheckbox
                      checked={
                        selectedPaths.size === (tree ? getAllDescendantFiles(tree).length : 0) &&
                        selectedPaths.size > 0
                      }
                      partial={
                        selectedPaths.size > 0 &&
                        selectedPaths.size < (tree ? getAllDescendantFiles(tree).length : 1)
                      }
                      onClick={() => {}}
                    />
              ...
              <div className="flex flex-col mt-4">
                ...
                <div className="flex flex-col gap-3 bg-white p-3 sm:p-4 border border-gray-200 rounded-md shadow-sm">
                  ...
                  {envRows.map((row, idx) => (
                    <div
                      key={row.id}
                      className="flex gap-2 sm:gap-3 items-center group flex-wrap sm:flex-nowrap"
                    >
                      <input
                        placeholder="Key"
                        value={row.name}
