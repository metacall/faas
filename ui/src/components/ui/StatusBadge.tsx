...
const STATUS_CONFIG: Record<StatusValue, { label: string; dot: string; text: string; animate: boolean }> = {
    ready: { label: 'Ready', dot: 'bg-[--color-status-ready]', text: 'text-[--color-status-ready]', animate: true },
    create: { label: 'Building', dot: 'bg-[--color-status-building]', text: 'text-[--color-status-building]', animate: true },
...
export function StatusBadge({ status }: StatusBadgeProps) {
    ...
    return (
        <span className={clsx('inline-flex items-center gap-1.5 text-xs font-medium', config.text)}>
            <span className={clsx('h-1.5 w-1.5 rounded-full', config.dot, config.animate && 'animate-pulse')} />
            {config.label}
        </span>
    );
}
