...
const LANG_CONFIG: Partial<Record<LanguageId | string, { label: string; bg: string; text: string }>> = {
...
export function LanguageBadge({ language }: LanguageBadgeProps) {
    const config = LANG_CONFIG[language] ?? { label: language, bg: 'bg-gray-800', text: 'text-gray-400' };

    return (
        <span className={clsx('inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium', config.bg, config.text)}>
            {config.label}
        </span>
    );
}
